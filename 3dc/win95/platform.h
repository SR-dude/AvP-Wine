/*$T platform.h GC 1.140 NO_DATE NO_TIME */
...
/*
 * Minimise header files to speed compiles... ;
 * adj
 */
...
/*
 -----------------------------------------------------------------------------------------------------------------------
    Video mode decsription (to be filled in by DirectDraw callback).
 -----------------------------------------------------------------------------------------------------------------------
 */
...
/* Maximum number of display modes that could be detected on unknown hardware. */
#define MaxAvailableVideoModes	100

/* #defines, structures etc for textprint system */
...
/*
 * Mouse handler modes (velocity mode is for interfacing to input functions that
 * need to read the mouse velocity, e.g. WeRequest functions, while poistion mode
 * is for menus and similar problems). The initial mode is defined as a compiled
 * in global in game.c
 */
typedef enum { MouseVelocityMode, MousePositionMode } MOUSEMODES;
 ...
 * palette calls within the DirectDraw interface system run on palette 0
 */
#define MaxPalettes 4

/* Video Mode Types */
typedef enum { VideoModeType_8, VideoModeType_15, VideoModeType_24, VideoModeType_8T } VIDEOMODETYPES;

/* Windows modes */
typedef enum { WindowModeFullScreen, WindowModeSubWindow } WINDOWMODES;
...
/* Dubious hack for dubious aspects of DirectDraw initialisation */
typedef enum
{
	/* Default */
	NoRestartRequired,
	...
	/*
	 * Characteristic of driver which will not support changing to a different bit
	 * depth without rebooting
	 */
	RestartDisplayModeNotAvailable,

	/*
	 * Characteristic of ModeX emulation leading to no video memory being available
	 * because of the need to cooperate with a large existing GDI surface which cannot
	 * be reduced without a reboot
	 */
	RestartOutOfVidMemForPrimary
} VIDEORESTARTMODES;

/*
 * Load modes for putting an image in a DirectDraw surface either do or do not
 * deallocate the system memory image, but always keep the DirectDraw one
 */
typedef enum { LoadModeDirectDrawOnly, LoadModeFull } IMAGELOADMODES;

/* Direct3D driver modes */
typedef enum { D3DSoftwareRGBDriver, D3DSoftwareRampDriver, D3DHardwareRGBDriver } D3DMODES;
...
typedef enum { RequestSoftwareRasterisation, RequestDefaultRasterisation } RASTERREQUESTMODES;

/* Actual scan draw mode */
typedef enum { ScanDrawDirectDraw, ScanDrawD3DRamp, ScanDrawD3DSoftwareRGB, ScanDrawD3DHardwareRGB } SCANDRAWMODES;

/* Z buffering request modes */
typedef enum { RequestZBufferNever, RequestZBufferAlways, RequestZBufferDefault } ZBUFFERREQUESTMODES;

/* Z buffering modes */
typedef enum { ZBufferOn, ZBufferWriteOnly, ZBufferOff } ZBUFFERMODES;

/* Request modes for DirectX memory usage */
typedef enum { RequestSystemMemoryAlways, RequestDefaultMemoryAllocation } DXMEMORYREQUESTMODES;

/* DirectX memory usage modes */
typedef enum { SystemMemoryPreferred, VideoMemoryPreferred } DXMEMORYMODES;

/*
 -----------------------------------------------------------------------------------------------------------------------
    .BMP File header ;
    Pack the header to 1 byte alignment so that the loader works (John's code, still under test).
 -----------------------------------------------------------------------------------------------------------------------
 */
...
typedef enum { TextureTypePGM, TextureTypePPM } TEXTUREFILETYPE;

/*
 * Windows initialisation modes. See InitialiseWindowsSystem in the default
 * win_proj.cpp for a full description.
 */
typedef enum { WinInitFull, WinInitChange } WININITMODES;
...
/*
 * Vertex sizes For copying vertices from item list polygons to triangles e.g.
 * Vertex 2 (x component) of the quad would be (for Item_Polygon) q[2 *
 * i_poly_vsize + i_vstart + ix] WARNING: If the item format changes these MUST be
 * updated
 */
#define vstart			4
...
BOOL FAR PASCAL		EnumDDObjectsCallback(GUID FAR *lpGUID, LPSTR lpDriverDesc, LPSTR lpDriverName, LPVOID lpContext);
#endif
