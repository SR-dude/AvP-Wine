...
void KillMarine(STRATEGYBLOCK *sbPtr, DAMAGE_PROFILE *damage, int multiple, SECTION_DATA *Section,VECTORCH *incoming) {
...
void MarineIsDamaged(STRATEGYBLOCK *sbPtr, DAMAGE_PROFILE *damage, int multiple, int wounds,SECTION_DATA *Section,VECTORCH *incoming)
{
	...
	if(sbPtr->SBDamageBlock.Health <= 0) {
		...
		KillMarine(sbPtr,damage,multiple,Section,incoming);
	} else {
