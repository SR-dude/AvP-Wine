/*$T consbind.cpp GC 1.140 NO_DATE NO_TIME */

/**
 *  DESCRIPTION: consbind.cpp Ability to kind keystrokes to strings so they appear in the console when you hit the key.
 *  Initial implementation went through the WM_KEYDOWN hook so that we get debouncing and typematic action for free;
 *  subsequently added an implementation based on the KeyboadInput[] array. AUTHOR: David Malcolm HISTORY: Created
 *  6/4/98
 */
	...
	(
		KeyBinding::ParseBindCommand
			(
				...
				/* returns where in the input string to continue processing */pProjCh_ToParse	/* ProjChar* pProjCh_In */
			)
	)
	{
		/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
// adj unused
		KeyBinding	*pNewBinding = new KeyBinding(theKey, pSCString_ToBind);
		/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
		...
		/* Feedback: */
		{
			...
			SCString	*pSCString_Feedback = new SCString(pSCString_1, pSCString_ToBind, pSCString_2, pSCString_3);
			/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
void KeyBinding::ParseUnbindCommand(ProjChar *pProjCh_ToParse)
{
	 ...
	 */
	{
		...
		if(bGotMatch)
		{
			...
			for(LIF < KeyBinding * > oi(&List_pKeyBindings); !oi.done();)
			{
				GLOBALASSERT(oi());
...
void KeyBinding::ListAllBindings(void)
{
	...
	for(CLIF < KeyBinding * > oi(&List_pKeyBindings); !oi.done(); oi.next())
	{
		GLOBALASSERT(oi());
		oi()->ListThis();
	}
...
void KeyBinding::Maintain(void)
{
	...
	if(IOFOCUS_AcceptControls())
	{
		 ...
		 * The list of pending SCStrings has been made static to the class in an attempt
		 * to ;
...
/*
 =======================================================================================================================
    private: ;
    Private ctor/dtor;
    to be called only by static fns of the class:
 =======================================================================================================================
 */
...
void KeyBinding::ListThis(void) const
{
	...
	SCString	*pSCString_Feedback = new SCString(pSCString_1, pSCString_2, pSCString_ToOutput, pSCString_3);
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
...
static SCString *GetMethodString(BindableKey inPhysicalKey)
{
	...
	if(GetKeyLabel(inPhysicalKey, theTextID /* TextID& outTextID */ ))
	{
...
(
	...
	/* returns where in the input string to continue processing */ProjChar	*pProjCh_In
)
{
