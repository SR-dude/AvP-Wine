...
/*
 * NOTES There are no pointers to strategy and/or animation data structures yet.
 * These will be added as and when they are needed.
 */
typedef enum { mtype_module, mtype_term } MODULETYPE;
...
typedef enum { vmtype_vmodule, vmtype_term } VMODULETYPE;
...
/*
 -----------------------------------------------------------------------------------------------------------------------
    This is the map block for module objects. It was originally based on the MAPBLOCK8 structure.
 -----------------------------------------------------------------------------------------------------------------------
 */
...
/*
 * Module functions called either when the module is visible or when the view is
 * inside the module.
 */
typedef enum { mfun_null, } MFUNCTION;
...
typedef struct module
{
	...
	MREF					m_ext;				/* Get module extents from the shape found through this other module */
	...
	VMODULE					*m_vmptr;			/* Pointer to an array of VMODULE, or "visible module" structures */

	MREF					*m_link_ptrs;		/* Pointer to an arbitrary sized array of module references - the array
												 * is zero terminated */

	/* should be got rid of soon */
	MODULETABLEHEADER		*m_table;			/* A hash table whose creation is triggered by a threshold value set by
												 * "system.h". This is to speed up module list traversal */
...
/* Flags */
#define m_flag_infinite				0x00000001	/* No extent test, the view is always in this module */

#define m_flag_gotptrs				0x00000002	/* Module references have been converted from names to pointers */

#define m_flag_open					0x00000004	/* The viewport/Door is open. This state is read from the "dptr"
												 * morphing frame if it is present and if it is appropriate to do so */
...
/*
 -----------------------------------------------------------------------------------------------------------------------
    "The View" The "View Finder" accesses the view location and orientation through this global structure. This is so
    that views can be passed to other functions as a single pointer if required.
 -----------------------------------------------------------------------------------------------------------------------
 */
