...
/* Maximum number shading tables. This equate might have to be moved to "system.h" */
...
/*
 * "shape.c" has been updated so that shapes without normals are outcoded using
 * the "scaled sprite" outcoding function, which looks at clip outcodes but does
 * not perform a back face cull.
 */
...
/*
 -----------------------------------------------------------------------------------------------------------------------
    ...
    BSP Block ;
    This struct is the one that ends up being walked. It is also used by z-trees, hence its unconditional inclusion.
 -----------------------------------------------------------------------------------------------------------------------
 */
...
/*
 -----------------------------------------------------------------------------------------------------------------------
    Shape Header Block ;
    Extra Item Data. As well as item extensions, items can have extra data in a parallel array. This data can be
    accessed using the normal index.
 -----------------------------------------------------------------------------------------------------------------------
 */
...
/*
 -----------------------------------------------------------------------------------------------------------------------
    it might be a good idea to put in an instruction field here so that each fragment can have an instruction telling
    it what to do e.g. a remain around instruction
 -----------------------------------------------------------------------------------------------------------------------
 */
...
{
	/* array of shape fragment indices terminated with ShapeIndex = NumFrags = -1 */
...
typedef struct	Adaptive_Degradation_Desc
{
	...
	int					distance;	/* The shape should be used if the distance is greater than or equal to this
									 * distance */

	/*
	 * KJL - some models are extremely low poly, and *only* work if they are drawn
	 * small on screen.
	 */
...
typedef struct	shapeheader
{
	...
	/*
	 * if shape_degradation_array is not null then it is terminated with an entry
	 * whose distance is 0 and whose shape is this shapeheader ;
...
/*
 * Polygon Header Block Polygon Data is as generic as any item. However it might
 * be useful to cast the initial fixed part of its data to the following struct.
 */
#define IHdrSize	4
#define ITrmSize	1
...
/*
 * Item Flags Some Item Flags can be shared, others are unique to a group of one
 * or more items.
 */
...
#define iflag_verticaledges		0x00000800	/* A collision option whereby the item is treated as if it is a prism of
											 * infinite extent formed by extrusion of its world xz projection in the
											 * y-axis */
...
#define iflag_bufferxy			0x08000000	/* Internal - Saturn Only - for xy clamped item */
#define iflag_clampz			0x10000000	/* Internal - Saturn Only - for z clamped item */
...
#define ih_flag_nochromakey 0x00000002					/* internal load flag indicating that d3_func should NOT set
														 * chroma keying for this image */
#define ih_flag_tlt			0x00000004					/* image pixels must be remapped through the tlt to get the
														 * screen palette entry */
#define ih_flag_16bit		0x00000008					/* in conjunction with ih_flag_tlt, the image is 16bit and the
														 * tlt has more entries to correspond */
...
/*
 -----------------------------------------------------------------------------------------------------------------------
    ...
    Texture Animation Header Structure The data is stored as an array of ints and so the pointer must be recast to this
    structure for access, just as item pointers must be.
 -----------------------------------------------------------------------------------------------------------------------
 */
...
/*
 -----------------------------------------------------------------------------------------------------------------------
    Texture Animation Frame Structure The header has a pointer to an array of these structures UV data for each frame
    is held in a separate int array
 -----------------------------------------------------------------------------------------------------------------------
 */
...
/*
 -----------------------------------------------------------------------------------------------------------------------
    Display Block Texture Animation Control Block An arbitrary number of these can be attached through a linked list to
    the display block.
 -----------------------------------------------------------------------------------------------------------------------
 */
